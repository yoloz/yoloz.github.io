...
function showArticleIndex() {
	...
	$("#article-toc-inner .toc").empty();
	$("#article-toc-inner .toc").append(show(h1List));

	// 点击目录索引链接，动画跳转过去，不是默认闪现过去
	$("#article-toc-inner .toc a").on("click", function (e) {
		e.preventDefault();
...
function pjaxLoad() {
	$(document).pjax('#tree a', '#article-container .article-inner', { fragment: '#article-container .article-inner', timeout: 8000 });
	$(document).pjax('#menu a', '#article-container .article-inner', { fragment: '#article-container .article-inner', timeout: 8000 });
	$(document).pjax('#briefList a', '#article-container .article-inner', { fragment: '#article-container .article-inner', timeout: 8000 });
	$(document).on({
